 ...
 *   3. .env com VIOS_USER, VIOS_PASS e (opcional) VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY para sync SIOE
...
if (!config.usuario || !config.senha) {
  console.error('Defina VIOS_USER e VIOS_PASS no .env antes de executar.');
  process.exit(1);
}
...
async function main() {
  ...
  if (process.env.VITE_SUPABASE_URL && process.env.VITE_SUPABASE_ANON_KEY) {
    ...
    try {
      ...
      console.log(`✅ Supabase SIOE atualizado. Upserted: ${result.upserted}, erros: ${result.errors}`);
      if (result.sama?.skipped) {
        console.log('ℹ️ SAMA: dual-write não configurado (SAMA_SUPABASE_URL / SERVICE_ROLE_KEY).');
      } else if (result.sama) {
        console.log(`✅ SAMA pessoas: ${result.sama.upserted} upserted, erros: ${result.sama.errors}`);
      }
    } catch (err) {
