 ...
 *   2. .env com: VITE_SUPABASE_URL=... e VITE_SUPABASE_ANON_KEY=... (SIOE)
 *      Dual-write SAMA (opcional): SAMA_SUPABASE_URL + SAMA_SUPABASE_SERVICE_ROLE_KEY
...
export async function runSync(filePath) {
  ...
  let sama = { upserted: 0, errors: 0, vinculados: 0, skipped: true };
  try {
    sama = await syncProcessosCompletoSama(rowsDedup);
  } catch (err) {
    console.error('[Sync SAMA] Falha no dual-write processos:', err.message);
    sama = { upserted: 0, errors: 1, vinculados: 0, failed: true };
  }

  return { upserted, errors, sama };
}
...
export async function runSyncTimeSheets(filePath) {
  ...
  let sama = { upserted: 0, errors: 0, vinculados: 0, skipped: true };
  try {
    sama = await syncTimesheetsSama(rowsDedup);
  } catch (err) {
    console.error('[Sync SAMA] Falha no dual-write timesheets:', err.message);
    sama = { upserted: 0, errors: 1, vinculados: 0, failed: true };
  }
...
/**
 * Cliente Supabase do projeto SAMA (dual-write).
 * Use service_role — o script roda no servidor e precisa escrever nas tabelas VIOS.
 */
function getSamaClient() {
  const url = process.env.SAMA_SUPABASE_URL;
  const key =
    process.env.SAMA_SUPABASE_SERVICE_ROLE_KEY ||
    process.env.SAMA_SUPABASE_ANON_KEY;
  if (!url || !key) return null;
  return createClient(url, key, { auth: { persistSession: false } });
}

async function upsertRowsToSama(table, rows, onConflict, label) {
  const sama = getSamaClient();
  if (!sama) {
    console.log(`[Sync SAMA] ${label}: SAMA_SUPABASE_URL / SAMA_SUPABASE_SERVICE_ROLE_KEY não definidos — dual-write ignorado.`);
    return { upserted: 0, errors: 0, skipped: true };
  }
  if (!rows.length) {
    return { upserted: 0, errors: 0, skipped: false };
  }
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    const { error } = await sama.from(table).upsert(chunk, { onConflict });
    if (error) {
      console.error(`[Sync SAMA] ${label} upsert error:`, error.message);
      errors++;
    } else {
      upserted += chunk.length;
    }
  }
  console.log(`[Sync SAMA] ${label} | Upserted:`, upserted, '| Erros:', errors);
  if (errors > 0) {
    console.error(`[Sync SAMA] ATENÇÃO: ${label} teve ${errors} lote(s) com falha no dual-write.`);
  }
  return { upserted, errors, skipped: false };
}

async function runSamaVinculacao(rpcName, label) {
  const sama = getSamaClient();
  if (!sama) return 0;
  const { data, error } = await sama.rpc(rpcName);
  if (error) {
    console.warn(`[Sync SAMA] Aviso vinculação ${label}:`, error.message);
    return 0;
  }
  console.log(`[Sync SAMA] ${label} vinculação:`, data ?? 0, 'linhas.');
  return data ?? 0;
}

/** Grava pessoas no SAMA (tabela public.pessoas — espelho SIOE). */
async function upsertPessoasSama(rowsDedup) {
  const valid = rowsDedup.filter((r) => r.ci != null && String(r.ci).trim() !== '');
  if (valid.length < rowsDedup.length) {
    console.warn('[Sync SAMA] Linhas sem CI ignoradas no dual-write:', rowsDedup.length - valid.length);
  }
  return upsertRowsToSama('pessoas', valid, 'ci', 'pessoas');
}

async function syncProcessosCompletoSama(rowsDedup) {
  const result = await upsertRowsToSama('processos_completo', rowsDedup, 'ci', 'processos_completo');
  if (result.skipped) return { ...result, vinculados: 0 };
  const vinculados = await runSamaVinculacao('processos_completo_vinculacao_pessoa', 'processos_completo');
  return { ...result, vinculados };
}

async function syncTimesheetsSama(rowsDedup) {
  const result = await upsertRowsToSama('timesheets', rowsDedup, 'ci', 'timesheets');
  if (result.skipped) return { ...result, vinculados: 0 };
  const vinculados = await runSamaVinculacao('timesheets_vinculacao_pessoa', 'timesheets');
  return { ...result, vinculados };
}

/**
 * Valida cabeçalho mínimo do CSV de pessoas/clientes do VIOS.
...
export function validatePessoasCsvHeader(headerRow) {
  ...
  if (idx.nome < 0) {
    throw new Error(
      'CSV não parece ser o relatório de clientes/pessoas: coluna "Nome" (coluna 4) não encontrada.'
    );
  }
  const norm = headerRow.map(normalizarColuna);
  const hasCi = norm.some((h) => h === 'ci' || h.includes('ci'));
  if (!hasCi && idx.ci !== 0) {
    console.warn('[Sync] Aviso: coluna CI não identificada claramente; usando posição fixa (coluna 1).');
  }
}
 ...
 * Dual-write: envia também para o projeto SAMA (tabela pessoas), se configurado.
...
export async function runSyncPessoas(filePathOrCsvContent) {
  ...
  // Dual-write: envia o mesmo conjunto para o projeto SAMA (tabela pessoas).
  let sama = { upserted: 0, errors: 0, skipped: true };
  try {
    sama = await upsertPessoasSama(rowsDedup);
  } catch (err) {
    console.error('[Sync SAMA] Falha no dual-write pessoas:', err.message);
    sama = { upserted: 0, errors: 1, failed: true };
  }
