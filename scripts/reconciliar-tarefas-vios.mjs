/**
 * Remove tarefas VIOS fora do mapa e reclassifica pendentes mapeadas.
 * Uso: node scripts/reconciliar-tarefas-vios.mjs
 */
import fs from "fs";
...
const { data: todas, error } = await supabase.from("vios_tarefas").select("*");
if (error) {
...
const rows = todas ?? [];
const foraMapa = rows.filter((t) => !tipoAtividadePorTarefaPai(t));
const noMapa = rows.filter((t) => tipoAtividadePorTarefaPai(t));

console.log(`Total: ${rows.length} · no mapa: ${noMapa.length} · fora do mapa: ${foraMapa.length}`);

if (foraMapa.length) {
  const ids = foraMapa.map((t) => t.id);
  const { error: delErr } = await supabase.from("vios_tarefas").delete().in("id", ids);
  if (delErr) {
    console.error("Erro ao remover fora do mapa:", delErr.message);
    process.exit(1);
  }
  console.log(`Removidas ${foraMapa.length} tarefa(s) fora do mapa.`);
}

// Reclassificar pendentes mapeadas (mesma lógica do script de classificação)
...
const alvo = (pendentes ?? []).filter(
  (t) => tipoAtividadePorTarefaPai(t) && t.usuario_concluiu_id
);

console.log(`Pendentes mapeadas para classificar: ${alvo.length}`);
...
for (const t of alvo) {
  ...
  if (updErr) {
    console.error("Erro ao vincular", t.ci, updErr.message);
    continue;
  }
...
console.log(`Classificadas agora: ${total}`);
